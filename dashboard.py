...
def build_dashboard():
    ...
    # NOTE: dashboard.html is a hand-crafted page; do NOT overwrite it.
    # The live_data.js written above contains all data dashboard.html needs.
...
new Chart(document.getElementById('ch').getContext('2d'),{{
  ...
  options:{{plugins:{{legend:{{display:false}}}},scales:{{y:{{min:0,max:5,ticks:{{color:'#5c5c5c',callback:v=>['','Low','Med','High','Crit',''][v]||''}},grid:{{color:'rgba(255,255,255,0.05)'}}}},x:{{ticks:{{color:'#5c5c5c',maxRotation:45}},grid:{{color:'rgba(255,255,255,0.05)'}}}}}}}}
    ...
    # dashboard.html is preserved — this legacy function is kept for reference only
    print("[OK] dashboard data available in live_data.js")
